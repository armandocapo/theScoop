...
function loadDatabase () {
  try {
    tempDatabase = yaml.safeLoad(fs.readFileSync('./database.yml', 'utf8'));
  } catch (e) {
      console.log(e);
  }
  if (tempDatabase) {
...
function saveDatabase () {
  try {
    fs.writeFileSync('./database.yml', yaml.safeDump(database, 4), 'utf8');
  } catch (e) {
      console.log(e);
  }
}
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body && requestComment.articleId && database.articles[requestComment.articleId] &&
...
function updateComment(url, request) {
  ...
  if (typeof savedComment === 'undefined' && !savedComment) {
    response.status = 404;
  } else if (typeof requestComment === 'undefined' && !requestComment || !requestComment.body) {
    response.status = 400;
  } else {

    database.comments[id].body = requestComment.body;

    response.body = {comment: requestComment.body};
    response.status = 200;
  }
...
//  const requestComment = request.body && request.body.comment;
  ...
  if (savedComment) {
    const userCommentIds = database.articles[savedComment.articleId].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    const userAuthorComments = database.users[savedComment.username].commentIds;
    userAuthorComments.splice(userAuthorComments.indexOf(id), 1);
